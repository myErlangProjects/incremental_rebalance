...
handle_info({node_data_changed, Znode}, #state{svr_name = SvrName,zk_chroot = Chroot, zk_znode = Znode, zk_chroot_children = ChildZNodes,zk_connection = Pid, zk_revoke_candidates = [], zk_assign_candidates = [], local_resource_list = PData, role = ?LEADER} = State) ->
	...
	{noreply, NewState};
handle_info({node_data_changed, Znode} = _Info, #state{svr_name = SvrName,zk_chroot = Chroot, zk_znode = Znode, zk_znode_suffix = ZnodeSuffix,zk_chroot_children = ChildZNodes,zk_connection = Pid, zk_revoke_candidates = RvkCandidates, zk_assign_candidates = AsgCandidates, local_resource_list = PData, role = ?LEADER} = State) ->
	{ok, {Data, _}} = erlzk:get_data(Pid, Znode),
	NData = binary_to_term(Data),
	RvkLinks = PData -- NData,
	AsgLinks = NData -- PData,
	...
	{noreply, NewState};
handle_info({node_data_changed, FZnode} = _Info, #state{svr_name = SvrName,zk_chroot = Chroot, zk_znode = _Znode, zk_znode_suffix = ZnodeSuffix,zk_chroot_children = ChildZNodes,zk_connection = Pid, zk_revoke_candidates = RvkCandidates, zk_assign_candidates = AsgCandidates, role = ?LEADER} = State) ->
...
handle_info({node_deleted,AdjLeader}, #state{svr_name = SvrName,zk_connection = Pid,zk_chroot = Chroot, zk_znode = Znode, zk_adj_leader = AdjLeader, role = ?FOLLOWER} = State) ->
	...
	NewState = leader_election(State#state{zk_chroot_children = lists:sort(Children)}),
	{noreply, NewState};
handle_info({node_data_changed, Znode}, #state{svr_name = SvrName,zk_znode = Znode,zk_connection = Pid, local_resource_list = PData, role = ?FOLLOWER} = State) ->
	{ok, {Data, _}} = erlzk:get_data(Pid, Znode),
	NData = binary_to_term(Data),
	RvkLinks = PData -- NData,
	AsgLinks = NData -- PData,
